...
const app = {
    ...
    showHome: function() {
        ...
        content.innerHTML = `
            <!-- Carrossel -->
            <div class="relative w-full max-w-4xl mx-auto h-56 md:h-96 rounded-3xl overflow-hidden mb-8 shadow-2xl">
                ${featured.map((p, i) => `
                <div class="carousel-slide absolute inset-0 opacity-0 transition-opacity duration-1000 ${i === 0 ? 'opacity-100' : ''}" style="background-image: url('${p.images[0]}'); background-size: cover; background-position: center;" onclick="app.showDetail(${p.id})">
    ...
    changePhoto: function(el, src) {
        ...
        document.querySelectorAll('.rounded-2xl').forEach(i => i.classList.replace('border-primary', 'border-transparent'));
        el.classList.replace('border-transparent', 'border-primary');
    }
};
